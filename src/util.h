    ...
    struct String {
        ...
        static std::vector<float> ToFloatVector(const std::vector<std::string>& stringVector) {
            ...
            for (const auto& str : stringVector) {
                auto num = static_cast<float>(atof(str.c_str()));
...
namespace MathUtil {
    ...
    [[nodiscard]] inline RE::NiPoint3 GetNiPoint3(const RE::hkVector4& a_hkVector4) {
        std::array<float, 4> quad{};
        _mm_store_ps(quad.data(), a_hkVector4.quad);
...
namespace PerkUtil {
    ...
    public:
        ...
        virtual BSContainer::ForEachResult Visit(RE::BGSPerkEntry* perk_entry) override {
            ...
            if (entry_point->functionData &&
                entry_point->entryData.function == RE::BGSEntryPointPerkEntry::Function::kMultiplyValue) {
...
namespace FormUtil {
    struct Parse {
        ...
        static RE::FormID GetFormIDFromMod(uint32_t relativeFormID, std::string modName) {
            ...
            if (!dataHandler)
                return static_cast<RE::FormID>(-1);
